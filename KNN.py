...
def test(): 
    ...
    sc = pickle.load(open('models/knn_scaler.pkl', 'rb'))                                            # load svm model scaler to apply correct scale
    X_test = sc.transform(X_test)                                                                    # scale features
    loaded_model = pickle.load(open('models/knn_scaler.pkl', 'rb'))                                   # load trained svm model   
...
#    grid_params = {
#        'n_neighbors': [2, 3, 5, 7, 9, 11, 15, 16],
#        'weights': ['uniform', 'distance'],
#        'metric': ['euclidean', 'manhattan']
#        }    
#    model = GridSearchCV(neighbors.KNeighborsClassifier(), grid_params, cv=5, n_jobs=-1)
#    model_fit = model.fit(X_train,y_train)
#    best_model = model_fit.best_estimator_
#    best_model.score(X_train,y_train)
#    predicted=best_model.predict(X_test)
    ### Best params found to be: {'metric': 'manhattan', 'n_neighbors': 16, 'weights': 'distance'} ###
    
    model = neighbors.KNeighborsClassifier(metric= 'manhattan', n_neighbors= 16, weights= 'distance')   # classifier with best parameters      
    model_fit = model.fit(X_train,y_train)                                                              # fit data to model
    model_fit.score(X_train,y_train)                                                                    # predict classes for test data
    ...
    # pickle.dump(model_fit, open('models/knn_model.sav', 'wb'))
    # pickle.dump(sc, open('models/knn_scaler.pkl', 'wb'))
...
# pickle.dump(best_model, open('models/knn_model.sav', 'wb'))
# pickle.dump(sc, open('models/knn_scaler.pkl', 'wb'))
